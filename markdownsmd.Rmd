...
write.csv(papers_df, "papers_df.csv", row.names = FALSE) # df papers (node)
...
head(Node_conference)
```


Edge_papers_conferences
```{r}
Edge_papers_conferences<- data.frame(
  id_paper=dblp_inproceedings$inproceedings.ID,
  ref_conf=dblp_inproceedings$crossref.string..
)
head(Edge_papers_conferences)
...
delete papaer that cite themself
...
```{r}
names(Node_papers_2)
names(Edge_papers_conferences)
left_join_df <- left_join(Node_papers_2,Edge_papers_conferences, by = c("id_paper"="id_paper"))
names(Node_conference)
left_join_df2 <- left_join(left_join_df,Node_conference, by = c("ref_conf"="ref"))
left_join_df2
names(left_join_df2)
# subset each of each conference 
unique(left_join_df2$acronym) # 20
```

join amb les keywords
```{r}
dblp_topics

left_join_df2_3 <- left_join(left_join_df2,dblp_topics, by = c("id_paper"="id_paper"))
# id paper, keywords i conferences

left_join_df2_list <- split(left_join_df2_3, left_join_df2_3$acronym)

ACML<-left_join_df2_list$ACML
names(ACML)


keywords_db_community <- c("data management", "indexing", "data modeling", "big data", "data processing", "data storage", "data querying")


# Filter the dataset to keep rows where at least one of the specified columns contains one of the specific values
filtered_df <- ACML %>%
  filter(
    (keyword_1 %in% keywords_db_community) | 
    (keyword_2 %in% keywords_db_community) | 
    (keyword_3 %in% keywords_db_community)
  )
filtered_df

names(filtered_df)
disjunction_df <- anti_join(ACML,filtered_df)

num_no_filter<-round(nrow(ACML)*0.1)
num_si_filter<-round(nrow(ACML)*0.9)

df_fin<-rbind(disjunction_df[1:num_no_filter,],filtered_df[1:num_si_filter,])
...
```{r}
apply_filter_and_mix <- function(df) {
  filtered_df <- df %>%
    filter(
      (keyword_1 %in% keywords_db_community) | 
      (keyword_2 %in% keywords_db_community) | 
  ...
  num_no_filter <- round(nrow(df) * 0.1)
  num_si_filter <- round(nrow(df) * 0.9)
  disjunction_df <- anti_join(df, filtered_df)
  df_fin <- rbind(disjunction_df[1:num_no_filter, ], filtered_df[1:num_si_filter, ])
  return(df_fin)
}
filtered_list <- lapply(left_join_df2_list,apply_filter_and_mix)

```


Next, we need to and the conferences and journals related to the database community
(i.e., are specic to the  eld of databases). Assume that if 90% of the papers published
...
filter_and_keep_90_percent <- function(df) {
  filtered_df <- df %>%
    filter(
      (keyword_1 %in% keywords_db_community) | 
      (keyword_2 %in% keywords_db_community) | 
  ...
  rows_to_keep <- round(0.9 * nrow(filtered_df))
  
  filtered_df <- head(filtered_df, rows_to_keep)
  
  return(filtered_df)
}

filtered_list <- lapply(left_join_df2_list, filter_and_keep_90_percent)
