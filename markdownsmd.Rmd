...
Edge between papers (journals) and authors: Edge_papers_author_1
```{r}
...
Edge between paper(journal) - journal: Edge_paper_journal
```{r}
...
papers_df <- rbind(Node_papers_1, papers_2)
...
relation paper (conferences) - conferences
```{r}
papers_conferences<- data.frame(
  id_paper=dblp_inproceedings$inproceedings.ID,
  ref_conf=dblp_inproceedings$crossref.string..
)

head(papers_conferences)
...
```{r}
names(papers_df)
Node_papers_1 # papers (journal)
papers_2 # papers (conference)
authors_1 #authors (journal)
authors_2 #authors (conference)
authors_paperID_rel # edge paper-author

# paper -conference
conference_df #conference
# paper -journal


```

community is de ned through
the following keywords: data management, indexing, data modeling, big data, data
processing, data storage and data querying.

agrupar papers per conference / per journal
