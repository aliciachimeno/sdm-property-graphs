...
```{r}
#filter columns
dblp_article <- dblp_article[,c(1,2,13,16,24,30,34,35)]# id, author, doi, journal, pages, title, volume, year
...
```{r}
papers_1 <- dblp_article[,c(1,6,3,5,8)]#falta abstract
```



# papers <-> conferences INPROCEEDINGS
...
authors_papersID_2 <- data.frame(
  id_paper = rep(dblp_inproceedings$inproceedings.ID, sapply(split_authors, length)),
  author = unlist(split_authors)
)

authors_papersID_2 <- unique(authors_papersID_2) # remove duplicate rows (id_paper, author)
...
authors_papersID_2 <- authors_papersID_2 %>%
  group_by(id_paper) %>%
  do(mark_main_author(.))

authors_2 <- unique(authors_papersID_2[,2])  # save only unique authors names

# write.csv(authors_papersID, "clean_datasets/authors_papersID.csv", row.names = FALSE) # df relation paperid-authors (edge)
# write.csv(authors, "clean_datasets/authors.csv", row.names = FALSE) # df authors (node)

```

Node papers: idpaper-title-DOI-year-abstract
```{r}
...
print(df_list)
#AI:
list<-as.list(df_list$AI["id_paper"])


for (i in 1:nrow(df_list$AI)) {
  random_numbers <- sample(list$id_paper, ncites)
}
```
...
df_list$AI$cites <- NA
for (i in 1:nrow(df_list$AI)) {
  id_paper <- df_list$AI[i, "id_paper"]
  ncites <- df_list$AI[i, "ncites"]
  random_numbers <- sample(df_list$AI$id_paper, ncites, replace = FALSE)
  print(random_numbers)
  df_list$AI[i, "cites"] <- paste(random_numbers, collapse = ",")
}
# separem les cites en columnes
df_list$AI <- separate(df_list$AI, cites, into = paste0("cites_", 1:5), sep = ",", fill = "right")
df_list$AI
